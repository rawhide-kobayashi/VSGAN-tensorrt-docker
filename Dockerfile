...
#vs-placebo
RUN apt-get update && apt-get install -y --no-install-recommends libplacebo-dev libplacebo208
...
RUN pip install vsdenoise havsfunc lvsfunc vsdeband glad2
...
RUN apt update && apt install  curl libjemalloc2 liblcms2-2 liblqr-1-0 libltdl7 -y
